...
Add the following code to the bottom of `main.groovy`. The purpose of this code is to break the response body up into individual lines and put those lines into an array. The `split()` method does this by splitting the string on each carriage return. 
